...
task :prepare_env => ['./venv/bin/python3', './agent/venv/bin/python3', 'client/pyproject.toml'] do
  sh 'sudo DEBIAN_FRONTEND=noninteractive apt-get install -y default-jre python3-venv libpq-dev libpython3-dev libldap-dev libsasl2-dev'
...
task :run_systests => PULUMI do
  ...
  # run system tests
  Dir.chdir('tests') do
    sh "KRAKEN_ADDR=#{kraken_addr} pytest-3 systests.py -s --junit-xml=systests-results.xml || true"
  end
