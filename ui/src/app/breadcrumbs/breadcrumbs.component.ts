...
export class BreadcrumbsComponent implements OnInit {
        ...
        this.breadcrumbsIn.subscribe(data => {
            ...
            for (let it of data) {
                ...
                case 'Branches':
                    ...
                    break
                case 'Results':
                    it2.url = '/branches/' + it.branch_id + '/' + it.flow_kind
                    ...
                    it2.menuItems = [{
                        ...
                        label: 'DEV',
                        routerLink: '/branches/' + it.branch_id + '/dev'
                    }]
            ...
            if (getBranches && this.currProjectId && (this.branches[this.currProjectId] === undefined || this.branches[this.currProjectId].length === 0)) {
            ...
            if (getFlows) {
                ...
                if (this.flows[flowsKey] === undefined || this.flows[flowsKey].length === 0) {
                    this.executionService.getFlows(this.currBranchId, this.currFlowKind, null, 10, this.currFlowId).subscribe(data => {
                        this.flows[flowsKey] = data.items
                    })
                }
            }

            if (getRuns && this.currFlowId && (this.runs[this.currFlowId] === undefined || this.runs[this.currFlowId].length === 0)) {
    ...
    showCrumbMenu(event, crumbMenu, breadcrumb) {
        ...
        case 'Projects':
            ...
            break
        case 'Branches':
            this.crumbMenuItems = this.branches[this.currProjectId].map(b => {
                return {label: b.name, routerLink: '/branches/' + b.id}
            })
            break
        case 'Flows':
            let flowsKey = '' + this.currBranchId + '-' + this.currFlowKind
